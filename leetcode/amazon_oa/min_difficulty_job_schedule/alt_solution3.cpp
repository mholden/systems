...
int minDifficulty(vector<int>& jd, int d) {
    ...
    for (int i = 0; i < jd.size() - days_left + 1; i++) {
        ...
        fe.difficulty = day_difficulty[0][i];
    ...
    while (!fifo.empty()) {
        ...
        if (fe.days_left == 1) {
            ...
            fe.difficulty += day_difficulty[fe.schedule[fe.schedule.size() - 2] + 1][fe.schedule[fe.schedule.size() - 1]];
            if (min < 0 || fe.difficulty < min)
                min = fe.difficulty;
        } else {
            ...
            for (int i = last + 1; i < jd.size() - days_left + 1; i++) {
                ...
                new_fe.difficulty = fe.difficulty + day_difficulty[last + 1][i];
                fifo.push(new_fe);
            }
        }
    }
